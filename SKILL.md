...
7. **不要默认把 Hermes cron 当成 UTC。** cron 表达式通常按调度器/机器本地时区解释。在北京时间部署中，`15 0,4,8,12 * * *` 实际是 00:15/04:15/08:15/12:15，不会跑 20:15。远行商人 08:15/12:15/16:15/20:15 北京时间应写成 `15 8,12,16,20 * * *`。如果迁移到其他机器，先用 `date` / cron `next_run_at` 验证调度时区，再决定是否换算。
